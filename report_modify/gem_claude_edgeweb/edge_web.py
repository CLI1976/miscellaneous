"""
edge_web.py — Playwright CDP 連接 Claude Edge 進行報告送出

架構：
  - 使用者手動開啟 Edge（--remote-debugging-port=9222）並登入
  - edge_web.py 透過 CDP 連入，偵測已登入的 Claude 分頁
  - 若無 Edge 或無 Claude 分頁，寫 error 退出
  - 瀏覽器頁面保持開啟，可直接繼續對話

啟動 Edge 指令（登入一次後 profile 會保留登入狀態）：
...
LOG_FILE  = os.path.join(SCRIPT_DIR, "edge_web.log")
...
async def run(report_text: str):
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.screenshot(path=os.path.join(SCRIPT_DIR, "edge_debug.png"))
            raise
